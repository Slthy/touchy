<script>
  ...
  import { getCookie, setCookie, fetchUserData } from '$lib/utils.js'
  let password, username, loginError = false
...
<div class="font-sans flex h-screen justify-center items-center p-16">
  ...
  <form on:submit={handleLogin} name="dataDiv" class="w-3/6 text-center p-8">
    <div class="flex flex-wrap items-stretch w-full mb-4 ">	
      <input type="text" bind:value={username} class="flex-shrink flex-grow flex-auto leading-normal w-px flex-1 border h-10 border-grey-light rounded rounded-l-none px-3 relative focus:border-blue focus:shadow" placeholder="Username" required>
    </div>
    ...
    {#if loginError}
      <span class="flex items-center font-medium tracking-wide text-red-500 text-xs mt-1 ml-1">Invalid username or password!</span>
    {/if}
  </form>
